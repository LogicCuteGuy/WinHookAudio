...
namespace wha {
constexpr uint32_t kWhaaMagic = 0x41414857u;  // 'WHAA' little-endian
constexpr uint32_t kWhaaVersion = 5u;
constexpr uint32_t kWhaaMaxPayload = 65507u;  // UDP safe
constexpr uint32_t kWhaaCodebookRedundant = 3u;
#define WHAA_MAGIC wha::kWhaaMagic
#define WHAA_VERSION wha::kWhaaVersion
#define WHAA_MAX_PAYLOAD wha::kWhaaMaxPayload
#define WHAA_CODEBOOK_REDUNDANT wha::kWhaaCodebookRedundant
...
constexpr bool IsValidWhaaChannels(uint32_t codec, uint32_t channels) {
  return IsValidNetworkChannels(static_cast<WHACodec>(codec), channels);
}
...
inline bool ValidateWhaaCodebook(const WHAACodebookHeader& h, std::string* error) {
  ...
  if (!IsValidWhaaStreamId(h.streamId)) return fail("streamId invalid");
  if (!IsValidSampleRate(h.sampleRate)) return fail("sampleRate invalid");
