...
#define WHA_MASTER_CLOCK_RATE_DEFAULT 48000u
#define WHA_MASTER_CLOCK_BITS_DEFAULT 32u
#define WHA_MASTER_CLOCK_BUFFER_DEFAULT 128u
...
constexpr bool IsBridgeInsidePool(WHASlotType type) {
  return !IsBridgeType(type) || (WHA_MAX == 512 && WHA_BRIDGE_COUNT == 4);
}
...
inline void SetSlotName(WHASlot& slot, const char* text) {
  ...
  std::size_t i = 0;
  for (; i + 1 < WHA_NAME_LEN && text[i] != '\0'; ++i) {
    slot.name[i] = text[i];
  }
  slot.name[i] = '\0';
  for (++i; i < WHA_NAME_LEN; ++i) {
    slot.name[i] = '\0';
  }
}
